local BRCore = exports['br-core']:GetCoreObject()
...
local function LoadInventory(source, citizenid)
	...
	for _, item in pairs(inventory) do
		if item then
			local itemInfo = BRCore.Shared.Items[item.name:lower()]
			if itemInfo then
...
local function SaveInventory(source, offline)
	local PlayerData
	if not offline then
		local Player = BRCore.Functions.GetPlayer(source)
...
local function AddItem(source, item, amount, slot, info)
	...
	local itemInfo = BRCore.Shared.Items[item:lower()]
	if not itemInfo and not Player.Offline then
		BRCore.Functions.Notify(source, "Item does not exist", 'error')
		return false
	end
	...
	if itemInfo['type'] == 'weapon' then
		info.serie = info.serie or tostring(BRCore.Shared.RandomInt(2) .. BRCore.Shared.RandomStr(3) .. BRCore.Shared.RandomInt(1) .. BRCore.Shared.RandomStr(2) .. BRCore.Shared.RandomInt(3) .. BRCore.Shared.RandomStr(4))
	...
	if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
		if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
			...
			TriggerEvent('br-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
		...
		elseif itemInfo['unique'] or (not slot or slot == nil) or itemInfo['type'] == 'weapon' then
			for i = 1, Config.MaxInventorySlots, 1 do
				if Player.PlayerData.items[i] == nil then
					...
					TriggerEvent('br-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. i .. '], itemname: ' .. Player.PlayerData.items[i].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[i].amount)
	...
	elseif not Player.Offline then
		BRCore.Functions.Notify(source, "Inventory too full", 'error')
	end
	return false
end
	...
	if slot then
		if Player.PlayerData.items[slot].amount > amount then
			...
			if not Player.Offline then
				TriggerEvent('br-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
			end
		...
		elseif Player.PlayerData.items[slot].amount == amount then
			...
			TriggerEvent('br-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
		...
		for _, _slot in pairs(slots) do
			if Player.PlayerData.items[_slot].amount > amountToRemove then
				...
				if not Player.Offline then
					TriggerEvent('br-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. Player.PlayerData.items[_slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[_slot].amount)
				end
			...
			elseif Player.PlayerData.items[_slot].amount == amountToRemove then
				...
				TriggerEvent('br-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
...
local function ClearInventory(source, filterItems)
	...
	TriggerEvent('br-log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** inventory cleared')
end
...
local function SetInventory(source, items)
	...
	TriggerEvent('br-log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** items set: ' .. json.encode(items))
end
...
local function CreateUsableItem(itemName, data)
	BRCore.Functions.CreateUseableItem(itemName, data)
end
...
local function GetUsableItem(itemName)
	return BRCore.Functions.CanUseItem(itemName)
end

exports("GetUsableItem", GetUsableItem)

---Use an item from the BRCore.UsableItems table if a callback is present
...
local function AddToStash(stashId, slot, otherslot, itemName, amount, info)
	amount = tonumber(amount) or 1
	local ItemData = BRCore.Shared.Items[itemName]
	if not ItemData.unique then
		if Stashes[stashId].items[slot] and Stashes[stashId].items[slot].name == itemName then
			...
			local itemInfo = BRCore.Shared.Items[itemName:lower()]
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		local itemInfo = BRCore.Shared.Items[itemData.name:lower()]
		...
		TriggerEvent("br-log:server:CreateLog", "drop", "New Item Drop", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) dropped new item; name: **"..itemData.name.."**, amount: **" .. itemAmount .. "**")
		...
		BRCore.Functions.Notify(source, Lang:t("notify.missitem"), "error")
	end
end
...
local function OpenInventory(name, id, other, origin)
    ...
    local Player = BRCore.Functions.GetPlayer(src)
	if ply.state.inv_busy then
		return BRCore.Functions.Notify(src, Lang:t("notify.noaccess"), 'error')
	end
	if name and id then
		local secondInv = {}
		if name == "stash" then
			if Stashes[id] then
				if Stashes[id].isOpen then
					local Target = BRCore.Functions.GetPlayer(Stashes[id].isOpen)
					if Target then
		...
		elseif name == "trunk" then
			if Trunks[id] then
				if Trunks[id].isOpen then
					local Target = BRCore.Functions.GetPlayer(Trunks[id].isOpen)
					if Target then
			...
			if (Trunks[id] and Trunks[id].isOpen) or (BRCore.Shared.SplitStr(id, "PLZI")[2] and (Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.type ~= "leo")) then
				secondInv.name = "none-inv"
		...
		elseif name == "glovebox" then
			if Gloveboxes[id] then
				if Gloveboxes[id].isOpen then
					local Target = BRCore.Functions.GetPlayer(Gloveboxes[id].isOpen)
					if Target then
		...
		elseif name == "otherplayer" then
			local OtherPlayer = BRCore.Functions.GetPlayer(tonumber(id))
			if OtherPlayer then
			...
			if Drops[id] then
				if Drops[id].isOpen then
					local Target = BRCore.Functions.GetPlayer(Drops[id].isOpen)
					if Target then
		...
		TriggerClientEvent("br-inventory:client:closeinv", id)
...
AddEventHandler('BRCore:Server:PlayerLoaded', function(Player)
	BRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "AddItem", function(item, amount, slot, info)
		return AddItem(Player.PlayerData.source, item, amount, slot, info)
	end)

	BRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "RemoveItem", function(item, amount, slot)
		return RemoveItem(Player.PlayerData.source, item, amount, slot)
	end)

	BRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemBySlot", function(slot)
		return GetItemBySlot(Player.PlayerData.source, slot)
	end)

	BRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemByName", function(item)
		return GetItemByName(Player.PlayerData.source, item)
	end)

	BRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemsByName", function(item)
		return GetItemsByName(Player.PlayerData.source, item)
	end)

	BRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "ClearInventory", function(filterItems)
		ClearInventory(Player.PlayerData.source, filterItems)
	end)

	BRCore.Functions.AddPlayerMethod(Player.PlayerData.source, "SetInventory", function(items)
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	local Players = BRCore.Functions.GetQBPlayers()
	for k in pairs(Players) do
		BRCore.Functions.AddPlayerMethod(k, "AddItem", function(item, amount, slot, info)
			return AddItem(k, item, amount, slot, info)
		end)

		BRCore.Functions.AddPlayerMethod(k, "RemoveItem", function(item, amount, slot)
			return RemoveItem(k, item, amount, slot)
		end)

		BRCore.Functions.AddPlayerMethod(k, "GetItemBySlot", function(slot)
			return GetItemBySlot(k, slot)
		end)

		BRCore.Functions.AddPlayerMethod(k, "GetItemByName", function(item)
			return GetItemByName(k, item)
		end)

		BRCore.Functions.AddPlayerMethod(k, "GetItemsByName", function(item)
			return GetItemsByName(k, item)
		end)

		BRCore.Functions.AddPlayerMethod(k, "ClearInventory", function(filterItems)
			ClearInventory(k, filterItems)
		end)

		BRCore.Functions.AddPlayerMethod(k, "SetInventory", function(items)
			SetInventory(k, items)
		end)
	end
end)

RegisterNetEvent('BRCore:Server:UpdateObject', function()
    if source ~= '' then return end -- Safety check if the event was not called from the server.
    BRCore = exports['br-core']:GetCoreObject()
end)
...
RegisterNetEvent('inventory:server:combineItem', function(item, fromItem, toItem)
	...
	local recipe = BRCore.Shared.Items[toItem.name].combinable

	if recipe and recipe.reward ~= item then return end
	if not recipeContains(recipe, fromItem) then return end

	TriggerClientEvent('inventory:client:ItemBox', src, BRCore.Shared.Items[item], 'add')
	AddItem(src, item, 1)
...
--	print('inventory:server:OpenInventory is deprecated use exports[\'br-inventory\']:OpenInventory() instead.')
	local src = source
	local ply = Player(src)
	local Player = BRCore.Functions.GetPlayer(src)
	if ply.state.inv_busy then
...
RegisterNetEvent('inventory:server:UseItemSlot', function(slot)
	...
	if not itemData then return end
	local itemInfo = BRCore.Shared.Items[itemData.name]
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if (fromInventory == "player" or fromInventory == "hotbar") and (BRCore.Shared.SplitStr(toInventory, "-")[1] == "itemshop" or toInventory == "crafting") then
		return
	end
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif BRCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				local playerId = tonumber(BRCore.Shared.SplitStr(toInventory, "-")[2])
				local OtherPlayer = BRCore.Functions.GetPlayer(playerId)
				...
				if toItemData then
					--Player.PlayerData.items[fromSlot] = toItemData
					local itemInfo = BRCore.Shared.Items[toItemData.name:lower()]
					toAmount = tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						RemoveItem(playerId, itemInfo["name"], toAmount, fromSlot)
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("br-log:server:CreateLog", "robbing", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** with player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
					end
				else
					local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("br-log:server:CreateLog", "robbing", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** to player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
				end
				local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
				AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info)
			elseif BRCore.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				local plate = BRCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("br-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("br-log:server:CreateLog", "trunk", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
				AddToTrunk(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif BRCore.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				local plate = BRCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("br-log:server:CreateLog", "glovebox", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
					local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("br-log:server:CreateLog", "glovebox", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
				AddToGlovebox(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif BRCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				local stashId = BRCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("br-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
					end
				else
					local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("br-log:server:CreateLog", "stash", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
				end
				local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
				AddToStash(stashId, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info)
			elseif BRCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				-- Traphouse
				local traphouseId = BRCore.Shared.SplitStr(toInventory, "_")[2]
				local toItemData = exports['br-traphouse']:GetInventoryData(traphouseId, toSlot)
				local IsItemValid = exports['br-traphouse']:CanItemBeSaled(fromItemData.name:lower())
				if IsItemValid then
					RemoveItem(src, fromItemData.name, fromAmount, fromSlot)
					TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
					if toItemData  then
						...
						toAmount = tonumber(toAmount) or toItemData.amount
						if toItemData.name ~= fromItemData.name then
							exports['br-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount)
							AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
							TriggerEvent("br-log:server:CreateLog", "traphouse", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
						end
					else
						local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("br-log:server:CreateLog", "traphouse", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
					end
					local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
					exports['br-traphouse']:AddHouseItem(traphouseId, toSlot, itemInfo["name"], fromAmount, fromItemData.info, src)
				else
					BRCore.Functions.Notify(src, Lang:t("notify.nosell"), 'error')
				end
			else
				-- drop
				...
				if toInventory == nil or toInventory == 0 then
					...
					if toItemData then
						...
						if toItemData.name ~= fromItemData.name then
							...
							TriggerEvent("br-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
						end
					else
						local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("br-log:server:CreateLog", "drop", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
					end
					local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
			...
			BRCore.Functions.Notify(src, Lang:t("notify.missitem"), "error")
		end
	elseif BRCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		local playerId = tonumber(BRCore.Shared.SplitStr(fromInventory, "-")[2])
		local OtherPlayer = BRCore.Functions.GetPlayer(playerId)
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					itemInfo = BRCore.Shared.Items[toItemData.name:lower()]
					toAmount = tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						RemoveItem(src, toItemData.name, toAmount, toSlot)
				...
				itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
				AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info)
			end
		else
			BRCore.Functions.Notify(src, Lang:t("notify.itemexist"), "error")
		end
	elseif BRCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		local plate = BRCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("br-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("br-log:server:CreateLog", "trunk", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("br-log:server:CreateLog", "trunk", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Trunks[plate].items[toSlot]
	...
	elseif BRCore.Shared.SplitStr(fromInventory, "-")[1] == "glovebox" then
		local plate = BRCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("br-log:server:CreateLog", "glovebox", "Swapped", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src..")* swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("br-log:server:CreateLog", "glovebox", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
					TriggerEvent("br-log:server:CreateLog", "glovebox", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = Gloveboxes[plate].items[toSlot]
	...
	elseif BRCore.Shared.SplitStr(fromInventory, "-")[1] == "stash" then
		local stashId = BRCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("br-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. stashId .. "*")
					else
						TriggerEvent("br-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. stashId .. "*")
					end
				else
					TriggerEvent("br-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. stashId .. "*")
				end
	...
	elseif BRCore.Shared.SplitStr(fromInventory, "-")[1] == "traphouse" then
		local traphouseId = BRCore.Shared.SplitStr(fromInventory, "-")[2]
		local fromItemData = exports['br-traphouse']:GetInventoryData(traphouseId, fromSlot)
		fromAmount = tonumber(fromAmount) or fromItemData.amount
		if fromItemData and fromItemData.amount >= fromAmount then
			local itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = GetItemBySlot(src, toSlot)
				exports['br-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], fromAmount)
				if toItemData then
					itemInfo = BRCore.Shared.Items[toItemData.name:lower()]
					toAmount = tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						RemoveItem(src, toItemData.name, toAmount, toSlot)
						exports['br-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
						TriggerEvent("br-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. traphouseId .. "*")
					else
						TriggerEvent("br-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. traphouseId .. "*")
					end
				else
					TriggerEvent("br-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. traphouseId .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = exports['br-traphouse']:GetInventoryData(traphouseId, toSlot)
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						itemInfo = BRCore.Shared.Items[toItemData.name:lower()]
						exports['br-traphouse']:RemoveHouseItem(traphouseId, toSlot, itemInfo["name"], toAmount)
						exports['br-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
					end
				end
				itemInfo = BRCore.Shared.Items[fromItemData.name:lower()]
			...
			BRCore.Functions.Notify(src, "Item doesn't exist??", "error")
		end
	elseif BRCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		local shopType = BRCore.Shared.SplitStr(fromInventory, "-")[2]
		local itemData = ShopItems[shopType].items[fromSlot]
		local itemInfo = BRCore.Shared.Items[itemData.name:lower()]
		local bankBalance = Player.PlayerData.money["bank"]
		local price = tonumber((itemData.price*fromAmount))

		if BRCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if BRCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				price = tonumber(itemData.price)
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					itemData.info.serie = tostring(BRCore.Shared.RandomInt(2) .. BRCore.Shared.RandomStr(3) .. BRCore.Shared.RandomInt(1) .. BRCore.Shared.RandomStr(2) .. BRCore.Shared.RandomInt(3) .. BRCore.Shared.RandomStr(4))
					itemData.info.quality = 100
					AddItem(src, itemData.name, 1, toSlot, itemData.info)
					TriggerClientEvent('br-drugs:client:updateDealerItems', src, itemData, 1)
					BRCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("br-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
				else
					BRCore.Functions.Notify(src, Lang:t("notify.notencash"), "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
					TriggerClientEvent('br-drugs:client:updateDealerItems', src, itemData, fromAmount)
					BRCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("br-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. "  for $"..price)
				else
					BRCore.Functions.Notify(src, "You don't have enough cash..", "error")
				end
			end
		elseif BRCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
				...
				AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
				TriggerClientEvent('br-shops:client:UpdateShop', src, BRCore.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
				BRCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
				TriggerEvent("br-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			BRCore.Functions.Notify(src, Lang:t("notify.noitem"), "error")
		end
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("br-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
					else
						TriggerEvent("br-log:server:CreateLog", "drop", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** - from dropid: *" .. fromInventory .. "*")
					end
				else
					TriggerEvent("br-log:server:CreateLog", "drop", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** -  dropid: *" .. fromInventory .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				toInventory = tonumber(toInventory)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
    ...
    local OtherPlayer = BRCore.Functions.GetPlayer(target)
    local dist = #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target)))
	if Player == OtherPlayer then return BRCore.Functions.Notify(src, Lang:t("notify.gsitem")) end
	if dist > 2 then return BRCore.Functions.Notify(src, Lang:t("notify.tftgitem")) end
	local item = GetItemBySlot(src, slot)
	if not item then BRCore.Functions.Notify(src, Lang:t("notify.infound")); return end
	if item.name ~= name then BRCore.Functions.Notify(src, Lang:t("notify.iifound")); return end
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info) then
				TriggerClientEvent('inventory:client:ItemBox',target, BRCore.Shared.Items[item.name], "add")
				BRCore.Functions.Notify(target, Lang:t("notify.gitemrec")..amount..' '..item.label..Lang:t("notify.gitemfrom")..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, BRCore.Shared.Items[item.name], "remove")
				BRCore.Functions.Notify(src, Lang:t("notify.gitemyg") .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('br-inventory:client:giveAnim', src)
				TriggerClientEvent('br-inventory:client:giveAnim', target)
			else
				AddItem(src, item.name, amount, item.slot, item.info)
				BRCore.Functions.Notify(src, Lang:t("notify.gitinvfull"), "error")
				BRCore.Functions.Notify(target, Lang:t("notify.giymif"), "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
			end
		else
			BRCore.Functions.Notify(src, Lang:t("notify.gitydhei"), "error")
		end
	else
		BRCore.Functions.Notify(src, Lang:t("notify.gitydhitt"))
	end
end)
...
BRCore.Functions.CreateCallback('br-inventory:server:GetStashItems', function(_, cb, stashId)
	cb(GetStashItems(stashId))
end)

BRCore.Functions.CreateCallback('inventory:server:GetCurrentDrops', function(_, cb)
	cb(Drops)
end)

BRCore.Functions.CreateCallback('BRCore:HasItem', function(source, cb, items, amount)
	--print("^3QBCore:HasItem is deprecated, please use BRCore.Functions.HasItem, it can be used on both server- and client-side and uses the same arguments.^0")
    local retval = false
    local Player = BRCore.Functions.GetPlayer(source)
...
BRCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType and invId then
		...
		elseif invType == "stash" then
			...
			BRCore.Functions.Notify(source,  Lang:t("notify.navt"), "error")
		end
	else
		BRCore.Functions.Notify(source,  Lang:t("notify.anfoc"), "error")
	end
end, "god")

BRCore.Commands.Add("rob", "Rob Player", {}, false, function(source, _)
	TriggerClientEvent("police:client:RobPlayer", source)
end)

BRCore.Commands.Add("giveitem", "Give An Item (Admin Only)", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, false, function(source, args)
	local id = tonumber(args[1])
	local Player = BRCore.Functions.GetPlayer(id)
	local amount = tonumber(args[3]) or 1
	local itemData = BRCore.Shared.Items[tostring(args[2]):lower()]
	if Player then
			if itemData then
				-- check iteminfo
				local info = {}
				...
				elseif itemData["type"] == "weapon" then
					amount = 1
					info.serie = tostring(BRCore.Shared.RandomInt(2) .. BRCore.Shared.RandomStr(3) .. BRCore.Shared.RandomInt(1) .. BRCore.Shared.RandomStr(2) .. BRCore.Shared.RandomInt(3) .. BRCore.Shared.RandomStr(4))
					info.quality = 100
				...
				elseif itemData["name"] == "labkey" then
					info.lab = exports["br-methlab"]:GenerateRandomLab()
				elseif itemData["name"] == "printerdocument" then
					info.url = "https://cdn.discordapp.com/attachments/870094209783308299/870104331142189126/Logo_-_Display_Picture_-_Stylized_-_Red.png"
				end

				if AddItem(id, itemData["name"], amount, false, info) then
					BRCore.Functions.Notify(source, Lang:t("notify.yhg") ..GetPlayerName(id).." "..amount.." "..itemData["name"].. "", "success")
				else
					BRCore.Functions.Notify(source,  Lang:t("notify.cgitem"), "error")
				end
			else
				BRCore.Functions.Notify(source,  Lang:t("notify.idne"), "error")
			end
	else
		BRCore.Functions.Notify(source,  Lang:t("notify.pdne"), "error")
	end
end, "god")


BRCore.Commands.Add("randomitems", "Give Random Items (God Only)", {}, false, function(source, _)
	local filteredItems = {}
	for k, v in pairs(BRCore.Shared.Items) do
		if BRCore.Shared.Items[k]["type"] ~= "weapon" then
	...
	for _ = 1, 10, 1 do
		...
		if AddItem(source, randitem["name"], amount) then
			TriggerClientEvent('inventory:client:ItemBox', source, BRCore.Shared.Items[randitem["name"]], 'add')
            Wait(500)
		end
	end
end, "god")

BRCore.Commands.Add('clearinv', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    local playerId = args[1] ~= '' and tonumber(args[1]) or source
    local Player = BRCore.Functions.GetPlayer(playerId)
    if Player then
        ClearInventory(playerId)
    else
        BRCore.Functions.Notify(source, "Player not online", 'error')
    end
end, 'god')
...
CreateUsableItem("driver_license", function(source, item)
	...
	local players = BRCore.Functions.GetPlayers()
